...
from ox1audio_backend.ingest.audio_tags import read_audio_tags
from ox1audio_backend.config import get_settings
from ox1audio_backend.services.covers import store_track_cover
from ox1audio_backend.db import SessionLocal, init_db
from ox1audio_backend.ingest.retry import (
    MAX_ML_ATTEMPTS,
    backoff_seconds,
    is_permanent_ml_error,
)
from ox1audio_backend.ingest.zip_safe import SafeAudioZip, ZipSafetyError
from ox1audio_backend.models import ImportJob, JobStatus, MlJob, Track, TrackStatus
from ox1audio_backend.services import artists as artist_svc
from ox1audio_backend import storage

logger = logging.getLogger(__name__)

# Match ml-worker threshold; used when analysis lacks is_instrumental (legacy rows).
_VOCALNESS_INSTRUMENTAL_MAX = 0.12


def _analysis_is_instrumental(analysis: dict | None) -> bool | None:
    if not analysis:
        return None
    if "is_instrumental" in analysis and analysis["is_instrumental"] is not None:
        return bool(analysis["is_instrumental"])
    if "vocalness" in analysis and analysis["vocalness"] is not None:
        try:
            return float(analysis["vocalness"]) < _VOCALNESS_INSTRUMENTAL_MAX
        except (TypeError, ValueError):
            return None
    scores = analysis.get("instrument_scores") or {}
    if not isinstance(scores, dict):
        return None
    try:
        voice = float(scores.get("voice") or 0.0)
    except (TypeError, ValueError):
        return None
    # Legacy: voice missing from top scores → treat as instrumental-leaning.
    return voice < _VOCALNESS_INSTRUMENTAL_MAX
            ...
            with SafeAudioZip(
                ...
                for entry in archive:
                    ...
                    try:
                        ...
                        session.add(track)
                        await session.flush()
                        await artist_svc.link_artists_from_tags(session, track, tags.artists)
...
async def process_ml_job(session: AsyncSession, job: MlJob) -> None:
    ...
    try:
        ...
        track.is_instrumental = _analysis_is_instrumental(result.analysis)
