...
from ox1audio_backend.models import Playlist, User
from ox1audio_backend.services import playlists as playlist_svc
from ox1audio_backend.tools.playlists import (
    CreatePlaylistArgs,
    GetPlaylistArgs,
    create_playlist,
    get_playlist,
...
async def test_get_playlist_tool_wrong_user_isolation() -> None:
    ...
    with patch(
        "ox1audio_backend.tools.playlists.playlist_svc.get_owned_playlist",
        new=AsyncMock(
...
async def test_api_get_playlist_ownership() -> None:
    ...
    try:
        with (
            patch(
                "ox1audio_backend.api.playlists.playlist_svc.get_owned_playlist",
                new=AsyncMock(return_value=playlist),
            ) as get_mock,
            patch(
                "ox1audio_backend.api.playlists.playlist_svc.ordered_track_ids",
                new=AsyncMock(return_value=[]),
            ),
            patch(
                "ox1audio_backend.api.playlists.hydrate_tracks",
                new=AsyncMock(return_value=[]),
            ),
        ):
